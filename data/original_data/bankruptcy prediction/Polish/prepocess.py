...
def process_table(data, mean_list):
    ...
    for j in range(len(data)):
        ...
        inference_query = "Human: \n" + f"{prompt}'{text}'" + " \nAnswer:\n\nAssistant: \n"
        # '0' is good (41314) and '1' is bad
        data_tmp.append(
            {
                'id': j, 
                "normal_query": f"{prompt}'{text}'" + ' \nAnswer:', 
                "chat_query": inference_query,
                'answer': answer, 
                "choices": ["no", "yes"],
                "gold": int(data[j][-1]), 
                'text': text
...
for i in range(1, 6):
    filename = os.path.join(curr_dir, f'{i}{name}')
...
print("The distribution of the saved data is : ", che)
...
os.makedirs('/Users/himanshu/Documents/Projects/CALM-train-TrustworthyNLP/data/split_data/Polish_bankruptcy_prediction', exist_ok=True)
for i in range(len(data)):
    json_save(data[i], save_name[i], directory='/Users/himanshu/Documents/Projects/CALM-train-TrustworthyNLP/data/split_data/Polish_bankruptcy_prediction')