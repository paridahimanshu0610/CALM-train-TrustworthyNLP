...
# Before splitting into train/dev/test, the raw data is first stratified-subsampled
# down to this fraction (matches the `test_size=0.8` kept-vs-dropped split below).
subsample_keep_fraction = 0.2
subsample_random_state = 100
split_random_state = 10086
...
bias_prompt_to_add = False           # add the debiasing note to ALL prompts (splits + bias)
counter_factual_prompt_to_add = False  # add the counterfactual note to ALL prompts (splits + bias)
total_per_group_samples = 60         # rows sampled per group for the feature-wise bias file

# Raw (non-prompted) bias CSVs for the full train/test split
train_bias_csv = 'TraIn_train.csv'
test_bias_csv = 'TraIn_test.csv'

# Feature-wise bias CSV (raw rows) + the jsonl/parquet name it's saved under
age_split_csv = 'travel_insurance_age_split.csv'
age_bias_name = 'travel_insurance_age_bias' + bias_prompt_file_extension
age_feature_index = -1  # 'Age' ends up last after 'Gender' is dropped in data_preparation
age_partition_value = 45  # age split point: "<= 45" vs "> 45"

# Output names for the main train/valid/test splits, in the order
# stratified_train_dev_test_split() returns them.
...
def save_bias_data(feature_size, test_data, train_data, directory='data'):
    ...
    tt_data.to_csv(os.path.join(directory, train_bias_csv), index=False, header=False)


def stratified_train_dev_test_split(data, train_size=0.7, dev_size=0.1, test_size=0.2,
                                     label_index=4, random_state=10086):
    """
...
#####process: load, stratified-subsample, and clean the raw data
data = pd.read_csv(name, sep=',', header=0, names=[i for i in range(feature_size)])
save_data, drop_data = train_test_split(data, test_size=(1 - subsample_keep_fraction), stratify=data[4],
                                         random_state=subsample_random_state)
che = get_num(save_data)
data = data_preparation(save_data.values.tolist())

#####process: split into train/dev/test (stratified on Claim status)
# NOTE: an earlier version used a manual seeded random-index split (matching the
# reference script's approach) instead of stratification. Kept here for reference:
# random.seed(10086)
...
train_data, dev_data, test_data = stratified_train_dev_test_split(
    data, train_size=train_size, dev_size=dev_size, test_size=test_size, random_state=split_random_state
)
...
save_bias_data(feature_size, test_data, train_data, directory=bias_data_dir)
...
#####process: feature-wise bias data (age)
age_split_df = save_featurewise_bias_data(test_data, feature_index=age_feature_index,
                                           n_samples_per_group=total_per_group_samples,
